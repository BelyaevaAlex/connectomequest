...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{generated/framework_diagram.png}
\caption{Certificate-carrying interaction loop. Graph and RGB tasks share
receipt-backed authorization and dependency-indexed revision, but not policies
or task semantics.}
...
fixed.
\input{generated/paired_coarse_main}
...
All four methods share the planner, action model, RGB observations, and update
schedule. A method-independent rule retains 513 of 800 generated layouts,
yielding 3,078 paired cells; every stored trace replays exactly.
...
The experiments separate three facts. Target granularity changes visible target
prevalence and witness acquisition under fixed wiring and budgets. No ordering is
uniformly best: random diversification is competitive on two graphs, while
learned second-hop ordering helps on HemiBrain. Certificate validity is
independent of search quality: weak policies may return no certificate, and
success requires active receipt support for every required claim.

The RGB study isolates evidence management. On long plans under sparse updates,
dependency indexing preserves complete-validation decisions while reducing
validation-and-replanning CPU time; unchecked reuse fails after final support
loss. It does not evaluate reinforcement learning, action efficiency, perceptual
correctness, or transfer of task-specific policies, planners, and claims.
Under trusted issuance, complete dependencies, and declared updates,
Proposition~\ref{prop:receipt-next-action} equates the two validation
procedures, not either with global replanning; it neither authenticates receipts
nor detects issuer error (\href{https://anonymous.4open.science/r/connectomequest/README.md}{anonymous code and artifacts}).
