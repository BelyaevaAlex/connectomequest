...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth,trim=0 90 0 55,clip]{generated/framework_diagram.png}
\caption{Certificate-carrying interaction loop. Proposals are mapped to required
claims and authorized only when active receipts support them; typed updates use
a dependency index to revalidate affected claims. Graph and RGB tasks share the
interface, not policies or semantics.}
...
The RGB study isolates evidence management, not broad robotics performance. On
long retained plans under sparse updates, dependency indexing preserves
complete-validation decisions while reducing validation-and-replanning CPU time;
unchecked reuse fails after final support loss. The study does not compare
model-free reinforcement learning, action efficiency, or perceptual correctness.
Only the interface transfers; policies, planners, and required-claim definitions
remain task-specific.

The guarantees assume a trusted issuer, complete dependencies, and declared
updates; they neither authenticate receipts nor detect issuer error.
Proposition~\ref{prop:receipt-next-action} equates the validation procedures
under its assumptions, but not either with global replanning.
\href{https://anonymous.4open.science/r/connectomequest/README.md}{Anonymous code and artifacts}.
